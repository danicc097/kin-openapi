...
func getValidationTests(t *testing.T) []*validationTest {
	...
	return []*validationTest{
		...
		{
			// FIXME:
			name: "error - correct deepobject query parameters",
			args: validationArgs{
			...
			wantErrSchemaReason: `value must be a boolean`,
			wantErrSchemaPath:   "/bool/0",
			wantErrSchemaValue:  "true",
			wantErrResponse: &ValidationError{
				Status: http.StatusBadRequest,
				Title:  `value must be a boolean`,
				Source: &ValidationErrorSource{Parameter: "deepFilter"},
			},
		},
		{
			name: "error - incorrect deepobject query parameters",
			args: validationArgs{
